...
class JellyGrab:
    def __init__(self, root):
        ...
        # Configurações
        self.config_file = "jellygrab_config.json"
        self.load_config()
        ...
        self.device_id = None
        self.series_data = []
        self.download_path = self.config.get("download_path", str(Path.home() / "Downloads" / "JellyGrab"))
    ...
    def download_worker(self):
        while True:
            ...
            if episode_id in self.cancelled_downloads:
                self.cancelled_downloads.remove(episode_id)
        ...
        def do_login():
            try:
                ...
                headers = {
                    ...
                    "X-Emby-Authorization": f'MediaBrowser Client="JellyGrab", Device="Python", DeviceId="{self.device_id}", Version="1.0.0"'
                }
    ...
    def download_episode_core(self, episode_id, show_success):
        ...
        try:
            ...
            is_cancelled = "cancelado" in str(e).lower()
            status_text = "🚫 Cancelado" if is_cancelled else "❌ Erro"
            item['status_var'].set(status_text)
            
            self.root.after(0, lambda: self.series_tree.item(episode_id, values=(
                self.series_tree.item(episode_id)["values"][0], "", status_text
            )))
            ...
            if is_cancelled:
                ...
                try:
                    ...
                    print(f"Erro ao remover arquivo cancelado: {err}")
                self.root.after(0, lambda: self.status_label.config(text=f"🚫 Download cancelado: {item['filename']}"))
            else:
    ...
    def cancel_download(self, episode_id):
        """Cancela um download em andamento ou na fila."""
        if episode_id in self.download_items:
            self.cancelled_downloads.add(episode_id)
            item = self.download_items[episode_id]
            item['status_var'].set("Cancelando...")
            
            # Atualiza a UI principal também
            self.root.after(0, lambda: self.series_tree.item(episode_id, values=(
                self.series_tree.item(episode_id)["values"][0], "", "🚫 Cancelando..."
            )))
    ...
    def update_manager(self):
        ...
        for ep_id, item in list(self.download_items.items()):
            if ep_id not in self.download_rows:
                ...
                def update_percent(var_name, index, mode, item_ref=item, label_ref=percent_label):
                    percent = item_ref['progress_var'].get()
                    label_ref.config(text=f"{percent:.1f}%" if item_ref['total_size'] > 0 else "...")
                item['progress_var'].trace_add("write", update_percent)
                ...
                ttk.Label(details_frame, textvariable=item['speed_var']).pack(side="left", padx=(0,10))
                
                ttk.Label(details_frame, text="ETA:").pack(side="left", padx=(0,5))
                ttk.Label(details_frame, textvariable=item['eta_var']).pack(side="left", padx=(0,10))
                
                ttk.Label(details_frame, text="Status:").pack(side="left", padx=(0,5))
                ttk.Label(details_frame, textvariable=item['status_var']).pack(side="left", padx=(0,10))

                cancel_btn = ttk.Button(details_frame, text="Cancelar", command=lambda eid=ep_id: self.cancel_download(eid))
                cancel_btn.pack(side="right", padx=5)
                item['cancel_btn'] = cancel_btn # Guardar referência
                
                self.download_rows[ep_id] = row_frame
        
        # Atualiza estado dos botões e total
        total_downloaded = 0
        total_size = 0
        total_speed = 0.0

        for ep_id, item in self.download_items.items():
            status = item['status_var'].get()
            if status in ("Concluído", "❌ Erro", "🚫 Cancelado"):
                if 'cancel_btn' in item and item['cancel_btn'].winfo_exists():
                    item['cancel_btn'].config(state="disabled")
            
            if status not in ("❌ Erro", "🚫 Cancelado"):
                total_downloaded += item['downloaded']
                total_size += item['total_size']
                try:
                    total_speed += float(item['speed_var'].get().split()[0])
                except:
                    pass
